...
const Skills = () => {
    ...
    const [hoverAchivIndex, setHoverAchivIndex] = useState(null);
  const skills = [
    {
      title: "Hello World!",
      level: "Common",
      ...
      >
        ...
        >
          {skills.map((item, index) => {
            return (
                <div
                className={`m-2 transition-transform ease-out ${hoverAchivIndex === index ? `transform scale-105` : ``}`}
                onMouseOver={()=>setHoverAchivIndex(index)}
                onMouseOut={()=>setHoverAchivIndex(null)}>
                <Skillbar
                  title={item.title}
                  level={item.level}
                  color={item.color}
